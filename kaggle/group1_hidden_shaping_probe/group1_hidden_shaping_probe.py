...
OUTPUT_DIR = Path("/kaggle/working/group1_hidden_shaping_probe")
REPORT_PATH = OUTPUT_DIR / "report.json"
HELPER_REPORT_PATH = OUTPUT_DIR / "helper_report.json"
HELPER_SCRIPT = OUTPUT_DIR / "run_group1_helper.py"
...
for index, word in enumerate(words, start=1):
    audio = synthesizer.tts(word, speaker_wav=str(model_dir / "conditioning_audio.wav"))
    if isinstance(audio, list):
        audio = np.asarray(audio, dtype=np.float32)
    sample_rate = getattr(config.audio, "sample_rate", 24000)
    wav_path = wav_dir / f"{{index:02d}}_{{word}}.wav"
...
def main() -> None:
    ...
    report["steps"].append({"name": "run_group1_helper", "result": helper_result})
    require_ok("run_group1_helper", helper_result)
