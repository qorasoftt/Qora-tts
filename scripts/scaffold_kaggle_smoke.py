...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a Kaggle smoke scaffold for cloud-to-cloud HF dataset validation."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("kaggle/train_smoke"),
        help="Directory where kernel metadata and the smoke script are written.",
    )
...
def main() -> int:
    ...
    metadata = {
        "id": f"{username}/qora-tts-train-smoke",
        "title": "Qora TTS Train Smoke",
        "code_file": "train_smoke_hf_to_kaggle.py",
        "language": "python",
        "kernel_type": "script",
        "is_private": "true",
        "enable_gpu": "false",
    ...
    (output_dir / "train_smoke_hf_to_kaggle.py").write_text(
        SMOKE_SCRIPT,
        encoding="utf-8",
    )
    return 0
